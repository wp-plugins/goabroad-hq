...
if ( $instance ) {
  ...
  $required = __( array('FirstName'=>false) , 'goabroadhq');
}
...
<div id="lead_capture_rows">
  ...
  <?php if(is_array($rows) && count($rows) > 0): ?>
    <?php foreach($rows as $row): ?>
      ...
      <select class="goabroadhq_select" data-required="<?php echo $widget->get_field_name('required'); ?>" name="<?php echo $widget->get_field_name('rows'); ?>[]">
...
<script type="text/javascript">
  ...
  if(!addRow){
    $(document).on('click','.add_row',function(){
      ...
      $('<div class="lead_capture_row"><button class="delete_row">Delete</button><select name='+key+' data-required='+required+' class="goabroadhq_select">'+rows+'</select><div class="goabroadhq_options"></div></div>').insertBefore(this);
    })
